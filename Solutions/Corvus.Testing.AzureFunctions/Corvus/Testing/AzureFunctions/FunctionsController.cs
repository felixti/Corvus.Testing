...
namespace Corvus.Testing.AzureFunctions
{
  ...
  public sealed class FunctionsController
  {
    ...
    public async Task StartFunctionsInstance(string path, int port, string runtime, string provider = "csharp",
...
StdErr: {StdErr}",
    ...
    private static async Task<(string, string)> GetCommandsByOperatingSystem()
    {
      if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
      {
        return ("cmd.exe", "/c npm prefix -g");
      }

      var hasNvm = await IsNvmInstalled().ConfigureAwait(false);
      if (hasNvm)
      {
        return ("sh", "");
      }
    ...
    private static async Task<bool> IsNvmInstalled()
    {
      // Running npm directly can run into weird PATH issues, so it's more reliable to run
      // cmd.exe, and then ask it to run our command - that way we get the same PATH
      // behaviour we'd get running the command manually.
      var processHandler = new ProcessOutputHandler(
        new ProcessStartInfo("sh", "-c '[ -d \"$HOME/.nvm\" ] && echo true || echo false'")
        {
          UseShellExecute = false,
          RedirectStandardOutput = true,
          RedirectStandardError = true,
      ...
      // We get a newline character on the end of the standard output, so we need to
      // trim before returning.
      return bool.Parse(processHandler.StandardOutputText.Trim());
    }
